...
- might be effective if we know some of the rules by which the passwords are generated
...
- This attack is effective we are appending characters at the end of the word and not at the beginning

Hybrid mode **7** prepends words from wordlist with the mask and tries to crack the hashes
- This attack is effective we are prepending characters 
...
- Though we used same wordlist for password generation and cracking, the success rate is very low. given,
    - we substituted the characters with other characters according the character map
    - we changed the first character to uppercase if it is an alphabet or appended a random uppercase alphabet to the beginning of the word
    - added a random mask to the end of the word to make the length of the word 14
...
- We achieved the above success rate by using a hybrid wordlist which got generated by character substitution
- Inorder to get a better safe password, we need to use our own character map and keep it secret
- It is fine to define your own rules or increase the length of the password